...
to allocate ressources (e.g. hire more service workers to adapt to the demand),
...
different from what we have seen so far. Specifically, the examples emphasise that,
unlike discrete random variables, the considered variables are 
...
can occur. This is done by defining the **Cumulative Distribution Function (CDF)**, which is sometimes called  **Probability Distribution Function**. 
...
Most of the PDF's that we are going to consider are bell-shaped. So, typically, we will have 
...
Similarly, for **continuous** random variables, we use integration
...
Very roughly speaking, the median is the value that splits the sample space in two intervals with equal cumulative probability. 

Let's illustrate these two location values in the pdf. We have purposefully chosen an assymetric (right)_skewed_ distribution to display the differences between these two values. 
...
There is a wide variety of Probability Distributions. In this section, we will explore the properties of the following: 
...
```{r ,echo = FALSE, fig.cap='', fig.align='center', out.width = "80%"}
  knitr::include_graphics("img/05_continuous_rv/uniform_pdf3-1.png")
```
...
```{r ,echo = FALSE, fig.cap='', fig.align='center', out.width = "80%"}
  knitr::include_graphics("img/05_continuous_rv/Unif_Diego-1.png")
```
...
```{r ,echo = FALSE, fig.cap='', fig.align='center', out.width = "80%"}
  knitr::include_graphics("img/05_continuous_rv/CDF_Diego-1.png")
```
...
- A Normal distribution is completely defined by its mean $\mu$ and its variance $\sigma ^{2}$. Infinitely many different normal distributions are obtained by varying the _parameters_ $\mu$ and $\sigma ^{2}$.
...
- "Bell-shaped": looks a little bit like a bell,
- Symmetric : it is not skewed either to the right or to the left,
- Unimodal : it has only one mode,
- The mean, median and mode are all equal,

Here we see a bunch of normal distributions, that we obtain by varying the parameters $\mu$ and $\sigma$:
...
The function $\phi_{(\mu,\sigma)}(x)$ does indeed define the pdf of a random variable with a mean of $\mu$ and a variance of $\sigma^2$. This was established by transforming from $X$ to $Z$ via the substitution $Z=(X-\mu)/\sigma$. Such a variable is said to be standardised. 

Note also that the resulting integrand:
$$
\frac{1}{\sqrt{2\pi}}\exp^{\{-\frac{z^2}{2}\}}=\phi_{(0,1)}(z)\,,
$$
is the pdf of a random variable $Z\sim \mathcal{N}(0,1)$. If $Z\sim \mathcal{N}(0,1)$ then $Z$ is called a **standard normal** random variable because: $\textsf{E}[Z]=0$ and $\textsf{Var}(Z)=1$
