...
equivalent to $0.683$, $0.954$ and $0.997$, 
respectively.
```{r ,echo = FALSE, fig.cap='', fig.align='center', out.width = "100%"}
  knitr::include_graphics("img/05_continuous_rv/Areas_Normal-1.png")
```

This leads to the rule: **... rule `68 -- 95 -- 99.7'** i.e. that if $X$ is a Normal random variable, $X \sim \mathcal{N}(\mu, \sigma^2)$, its realization has approximately a probability of:

| $\bullet$ | $68 \, \%$ | \mbox{of being in the interval} | $\lbrack \mu - \sigma, \, \mu + \sigma \rbrack$;|
| $\bullet$ | $95 \, \%$   |\mbox{of being in the interval} |$\lbrack \mu - 2 \, \sigma, \, \mu + 2 \, \sigma \rbrack$; |
| $\bullet$ | $99.7 \, \%$ | \mbox{of being in the interval}| $\lbrack \mu - 3 \, \sigma, \, \mu + 3 \, \sigma\rbrack$.|
...
Moreover, the sum of independent exponential random variables has a Gamma distribution (see tutorial).
```
